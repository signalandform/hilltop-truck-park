...
export default function AdminVendorsList() {
  ...
  useEffect(() => {
    supabase
      ...
      .select("id, name, email, food_truck_name, is_read, created_at")
  ...
  const handleDelete = async (id: string) => {
    ...
    await supabase.from("cms_vendor_submissions").delete().eq("id", id);
    ...
    <>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-500 text-sm">No vendor requests yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 font-medium text-slate-600">Truck Name</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr
                  ...
                  onClick={() => router.push(`/admin/vendors/${item.id}`)}
                >
                  ...
                  <td className="px-4 py-3 text-slate-700">{item.food_truck_name ?? "—"}</td>
